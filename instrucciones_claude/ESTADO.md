...
**🔥 NUEVO 2026-06-22 (d) — sync idempotente de roles/permisos en cada `up`.** Nuevo `keycloak/sync-roles.sh` + servicio `keycloak-config` (init-container kcadm): en cada `docker compose up` asegura client-roles + realm-roles + composites **sin tocar el resto del realm** (resuelve que `--import-realm` no re-importe un realm existente, sin `down -v`). **Aplicado al local y verificado**: `SECRETARIA_RM` creado + VENDEDORA con `books`/`book_sales`. Ojo: roles viven en 2 lados (realm JSON para 1er import + `sync-roles.sh` para re-sync) → mantener en sync; prod necesita admin con `manage-realm`+`manage-clients`. Detalle DIARIO 06-22 (d).

**🔥 NUEVO 2026-06-22 (c) — cerrado el resto del backlog P0 12-jun (mío).** (3) **3ra opción de cuotas**: enum `InstallmentDistribution {SEPARATE, TOTAL, COURSE_AND_FEE}` (reemplaza el booleano); la nueva `COURSE_AND_FEE` = curso+matrícula en cuotas, libros aparte. Generator+service+EnrollmentForm (select de 3) deployados. (4) **Roles**: realm JSON con VENDEDORA + `books`/`book_sales` y nuevo `SECRETARIA_RM` + `APP_ROLES`. **No aplican en el Keycloak local** (`--import-realm` no re-importa si el realm existe) — aplican en arranque fresco (test-server) o re-import (down -v wipea data) o a mano por consola; en local no wipeé. (5) Fixes de comisión (mapper SET_TO_NULL + @Positive). Build rc=0, backend limpio, tsc verde. **P0 del 12-jun cerrado de mi lado** (email = tercero). **Restante:** aplicar realm nuevo al desplegar + **#2 test suspensión automática Moodle** (sobre acceso3, después; OJO: no hacer down -v o se pierde acceso3 vinculado). Detalle DIARIO 06-22 (c).

**🔥 NUEVO 2026-06-22 (b) — implementados 2 ítems P0 del backlog 12-jun (deployado + verificado).** (1) **Cursos**: fuera "fecha de examen" (UI); **modalidad en cascada por unidad** (FS = Diplomatura Prematuros/Neurodesarrollo/Curso PAF; resto Residencias); **comisión para FS** (V030 `courses.commission` + entidad + DTOs + mapper; en el form FS muestra Comisión+Año). (2) **Estado del alumno = Moodle** (#1): saqué Activo/Inactivo; grilla/detalle muestran "Falta dar de alta en Moodle" (sin moodleUserId) o activo/suspendido (vinculado). V030 aplicó OK, backend arrancó limpio, `tsc -b` verde. Code-review: el "critical" era falso (target/ stale del host; el build de Docker regenera el mapper y pasó). **Pendiente menor** (agrupar con próximo backend): `commission` SET_NULL al editar fuera de FS + `@Positive`. **Backlog P0 restante:** rol `SECRETARIA_RM` + vendedora con stock/ventas de libros (Keycloak+front); 3ra opción de cuotas (curso+matrícula en cuotas, libros aparte). Luego #2 (test suspensión automática). **Email = lo hace un tercero, fuera de mi scope.** Detalle en DIARIO 06-22 (b).

**🔥 NUEVO 2026-06-22 — procesada la transcripción de la reunión 12-jun (demo completa). Reprioricé: cerrar para servidor de prueba.** Doc nueva `14-requerimientos-reunion-20260612.md` (+ PDF `transcripcion-reunion-20260612.pdf`). Reunión muy positiva (felicitaciones de Jaque/Gustavo); lo que salió son **refinamientos**, no features grandes. **Consigna: cerrar lo demostrado + email + Moodle → subir a servidor de prueba con datos reales → producción.** **P0 nuevo:** (1) **Email** = la pieza que gatilla el go-live (provider EnvíaloSimple o SMTP; recibo+contrato PDF en alta; alerta día 1); (2) **Moodle** alta/baja (esperando permisos de David); (3) **Cursos**: sacar `fecha de examen`, modalidad **en cascada por unidad** (FS = tipos de diplomatura, Jaque manda nombres; Residencias = actuales), y **ciclo lectivo → comisión** para FS (comisión 10 actual, 11 arranca ago-2026 — reaparece el concepto Commission diferido en Fase 9); (4) **Inscripción**: 3ra opción de cuotas (curso+matrícula en cuotas, **libros aparte**); (5) **Roles**: nuevo `SECRETARIA_RM` + vendedora con stock/ventas de libros; (6) **deploy a servidor de prueba**. **P1:** liquidación inputs %/$ por ítem + administración monto + **fórmula detallada que pasa Nico**; selector de promo en inscripción y venta; venta de varios libros sueltos en una operación + auto-descuento por alumno/sin-alumno; selector de mensajes WhatsApp predeterminados; dashboard rojo más intenso >1.5 mes; medios de pago (lista de Nico); "Royalties"→"Autorías". **P2/futuro:** módulo seguimiento académico Moodle (notas), presupuesto en 5 plazos, plataforma de cobro del banco. **Bloqueantes = data del cliente** (modalidades FS, fórmula liquidación, textos mail/WhatsApp, PDFs contrato/recibo, medios de pago, autores no-residencias). **Pendientes Fran en DIARIO 06-22.** Detalle completo en doc 14.

**🔥 NUEVO 2026-06-18 — Moodle cerrado end-to-end (back + front, feature-flagged).** Llegó el token de David + sus 2 funciones (`core_user_get_users_by_field` lectura, `core_user_update_users` suspender/activar a nivel CUENTA). Completé sobre el módulo `modules/moodle/` ya existente: (1) **flag `MOODLE_AUTO_SUSPEND_ENABLED` (default false)** separado de `MOODLE_ENABLED` — gatea la suspensión automática del scheduler; permite prender la integración sin suspender morosos reales en masa. (2) **`GET /moodle/lookup?email=`** (read-only, sin persistir) para el botón "Validar con Moodle" del alta. (3) **`GET /moodle/students/{id}/account`** (estado vivo suspended). (4) **`GET /moodle/unlinked-students`** (alumnos sin vincular + cursos, JOIN FETCH) para el export. (5) **`moodleUserId` opcional en `StudentCreateRequest`** (persiste el vínculo al crear). (6) suspend/activate → 204; `MoodleException` → 502. **Front (área Fran, habilitado por el usuario para Moodle):** botón "Validar con Moodle" inline en el alta (`StudentForm`), Suspender/Reactivar manual + badge en `StudentDetail`, export "No vinculados (Moodle)" en `Alumnos`, y `api/moodle.ts`/`types/student.ts`. **Authorities (decisión usuario):** todo gateado por `students:read`/`students:write` (no `moodle:*`) — quien da de alta alumnos puede hacer Moodle, sin tocar Keycloak. **Verificado:** backend compila JDK21 (host tiene 17, compilé en contenedor) + `tsc -b` limpio + code-review 0 críticos (2 HIGH fixeados). **Sin migración** (`moodle_user_id` ya en V002). **Sin commitear aún.** Decisiones con el usuario: reactivación al pagar = **manual**; rollout contra Moodle PROD (`www.imedba.com.ar`) por pasos (prender+`link-all` read-only → test write 1 cuenta de David → flip auto-suspend). La nota vieja "badge Moodle por inscripción" queda superada: la suspensión de David es per-CUENTA, así que el estado es per-alumno (StudentDetail). Detalle completo en DIARIO 06-18.
...
**🔥 2026-06-12 (b) — Dashboard migrado a endpoints reales (activity + overdue).** Nuevo `api/dashboard.ts` con `summary()`/`activity()` + `installmentsApi.overdue()`. `Dashboard.tsx` ahora consume `GET /dashboard/activity` (1 request, sin caps) y `GET /installments/overdue` (status OVERDUE && diasVencidos > 10, ya ordenado). Las 4 cards de refuerzo positivo (ingresos+delta, alumnos nuevos, inscripciones, libro top) siguen client-side porque `/dashboard/summary` solo expone totales del sistema — si Santi extiende `summary` con esos campos las migro a 1 request.

**🔥 2026-06-12 (a) — sincronización con V028 pool de autorías.** `types/book.ts` y `types/book-sale.ts` actualizados con `royaltyPoolPct`. `BookForm` expone el campo "% venta a autorías (pool)" (placeholder 10). `BookDetail` muestra la frase explicativa del reparto. **Auditado**: la agrupación cliente-side de Royalties (`Ventas.tsx` tab Royalties) solo suma `royaltyAmount` tal cual viene del back, no re-multiplica → queda alineada con la nueva semántica `venta × pool% × %autora`.

**🔥 2026-06-10 — botón Exportar en TODAS las grillas.** 11 grillas con `Exportar` nuevo (Alumnos/Cursos/Inscripciones/Descuentos/Diplomaturas/Liquidaciones/Autores/Libros/Colecciones/Personal/Ventas-tab-Ventas). Respetan filtros y sort, fetchean `size=2000`, bajan `<entidad>-YYYY-MM-DD.csv` (BOM UTF-8 + `;` — abre directo en Excel es-AR). Header pattern: `<div flex gap=0.5rem>` con Exportar + acción principal.

**🔥 2026-06-09 — PENDINGS de Santi cerrados.** (1) `BookForm.tsx` solo muestra stock en alta; en edición el ajuste va por `BookDetail` con mini-form `PUT /books/{id}/stock` (motivo opcional). (2) `InstallmentStatus` + `CANCELLED` con badge gris ("Anulada"); `isPayable` ya solo es PENDING/OVERDUE. (3) Errores 409 ahora salen por SweetAlert (`alertError`) en vez de `alert()`/`window.confirm`. (4) Cuotas: cleanup `handleWaiveSurcharge` con `confirmAction`. (5) **Saqué TODAS las validaciones de rango numéricas arbitrarias** del SPA — 12 forms ahora solo verifican `Number.isNaN(Number(v))`; nada de "entre 0 y 1000".
