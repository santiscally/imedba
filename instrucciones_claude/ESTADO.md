...
**🔥 2026-06-12 — sincronización con V028 pool de autorías.** `types/book.ts` y `types/book-sale.ts` actualizados con `royaltyPoolPct`. `BookForm` expone el campo "% venta a autorías (pool)" (placeholder 10). `BookDetail` muestra la frase explicativa del reparto. **Auditado**: la agrupación cliente-side de Royalties (`Ventas.tsx` tab Royalties) solo suma `royaltyAmount` tal cual viene del back, no re-multiplica → queda alineada con la nueva semántica `venta × pool% × %autora`. Pending: migrar Dashboard a `/dashboard/{summary,activity}` + `/installments/overdue` reales (hoy compuesto client-side por malentendido del 06-09).
